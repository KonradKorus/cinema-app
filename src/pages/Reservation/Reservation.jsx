...
import {getScreeningById, getReservedBookSeats, addReservation, deleteScreening} from "../../hooks/hook";
...
const Reservation = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async() =>{
      ...
      if(data){
        ...
        setHour(data.start_time.split('T')[1].split(':')[0] + ':' + data.start_time.split('T')[1].split(':')[1])
        if(event.translation === "SUBTITLES")
          setTranslation("Napisy")
        if(event.translation === "DUBBING")
          setTranslation("Dubbing")
        if(event.translation === "VOICE_OVER")
          setTranslation("Lektor")
      }
  ...
  const isSeatReserved = (row, col) => {
    return reservedSeats.some(seat => seat.row_number === row && seat.seat_number === col);
  };
  ...
  return (
    <div>
      <div>
        <h1 style={{ textAlign: 'center', fontSize: '36px' }}>Wybierz miejsca</h1>
        <Container sx={{ marginLeft: '21.8%', textAlign: 'left', marginBottom:'85px' }}>
            <ul>
                <li style={{ listStyleType: 'none', cursor: 'pointer' }}>
                    <Container style={{ display: 'flex' }}>
                        <p>
                            Tytuł: {movie.title} <br></br> <br></br>
                            {translation}, {event.image_format}<br></br><br></br>
                            {date} {hour} <br></br><br></br>
                            Numer sali: {event.room_number}

                        </p>
                        <Link style={{ paddingLeft: 540}} to={`/movie-description/${movie.id}`}>
                            <img src={movie.image} alt={movie.title} style={{ width: '180px', height: 'auto', marginTop: '13px'}} />
                        </Link>
                    </Container>
                </li>
                <div style={{ width: '78%', height: '60px', marginTop: '2%', backgroundColor: 'lightgray', display: 'flex', color: 'black', justifyContent: 'center', alignItems: 'center'}}>
                    <span>Ekran</span>
                </div>
            </ul>
        </Container>
          <Container sx={{ marginLeft: '24%', textAlign: 'left', marginBottom: 10 }}>
              {Array.from({ length: 6 }, (_, row) => (
                          ...
                          return (
                              <Button
                                  ...
                                  sx={{
                                      ...
                                      '&.Mui-disabled': {
                                          backgroundColor: block === 'disabled' ? 'red' : buttonColor
                                      }
                                  }}
                                  disabled={isReserved}
                  ...
                  <Button variant="contained" style={{marginLeft: '63.5%', marginTop: '30px'}} onClick={handleSaveClick}>
                      Zarezerwuj
                  </Button>
